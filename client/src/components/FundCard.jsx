...
const FundCard = ({ owner, title, description, target, deadline, amountCollected, image, handleClick, filter }) => {
  const remainingDays = daysLeft(deadline)
  if (filter === "Active Campaigns") {
    if(remainingDays <= 0) return
  } else if (filter === "Expired Campaigns") {
    if(remainingDays > 0) return
  }
  return (
